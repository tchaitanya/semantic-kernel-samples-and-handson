...
{
    public const string DefaultConfigFile = "config/AzureOpenAIConnectorConfig.json";
    ...
    /// <summary>
    /// Load the kernel settings from <config.json> if the file exists and if not attempt to use user secrets.
    /// </summary>
    internal static KernelSettings LoadSettings(string filepath = "")
    {
        try
        {
            ...
            Console.WriteLine($"Semantic kernel connector config file '{filepath}' not found, attempting to load configuration from user secrets.");
